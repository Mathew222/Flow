...
export const generatePosterContent = async (
  ...
  1. ${brandName ? `USE EXACTLY THIS BRAND NAME: "${brandName}" - DO NOT change it or use a different name!` : 'Generate a creative brand name if none provided.'}
  2. ${customSlogan ? `USE EXACTLY THIS SLOGAN: "${customSlogan}" - DO NOT change it!` : 'Generate a creative slogan if none provided.'}
  ...
  1. brand_name (string) - ${brandName ? `MUST BE EXACTLY: "${brandName}"` : 'generate one'}
  2. short_slogan (max 8 words) - ${customSlogan ? `MUST BE EXACTLY: "${customSlogan}"` : 'generate one'}
...
export const enhanceProductImage = async (
  ...
  const visualDirectives = `
    ...
    6. ⚠️ FULL BLEED - NO BORDERS:
       - The image MUST be full bleed - background extends to ALL edges
       - NO white borders, margins, or frames around the image
       - NO blank space at the edges
       - The background/environment should fill the ENTIRE image canvas
       - Edge to edge coverage - absolutely NO white or empty space around the image
    ...
    OUTPUT: Generate a single stunning, high-quality FULL BLEED image (no borders) with the FULL product visible and the EXACT appearance as the reference.
  `;

  try {
    console.log("Starting image generation with model: gemini-2.0-flash-exp");
    ...
    console.warn("No image found in AI response parts, using original image");
